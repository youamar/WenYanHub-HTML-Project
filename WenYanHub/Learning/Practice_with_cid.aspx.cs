...
{
    public partial class Practice_with_cid : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (int.TryParse(Request.QueryString["cid"], out int targetCid))
                {
                    BindPracticesByCid(targetCid);
                }
                else
                {
                    Response.Redirect("~/Learning/PracticeSelection.aspx");
                }
            }
        }

        private void BindPracticesByCid(int cid)
        {
            ...
            using (var db = new AppDbContext())
            {
                ...
                var filteredData = (from p in db.Practices
                                    ...
                                    where c.ContentId == cid && p.DueDate > now
                                    select new
                                    {
                                        p.PracticeId,
                                        p.Description,
                                        ...
                                        //Capture the file paths uploaded by students
                                        StudentFile = sub != null ? sub.StudentAnswerFileUrl : "",
                                        IsHidden = sub != null && sub.IsFeedbackHidden
                                    }).ToList();

                rptSpecificPractices.DataSource = filteredData;
                rptSpecificPractices.DataBind();

                pnlNoData.Visible = (filteredData.Count == 0);

                if (filteredData.Any())
                {
                    litArticleName.Text = filteredData.First().ArticleTitle;
                }
            }
        }
            ...
            <div class='card mb-3 shadow-sm border-0 rounded-4 overflow-hidden'>
                <div class='card-body p-4 d-flex align-items-center justify-content-between hover-list-item' 
                    ...
                    <div class='flex-grow-1'>
                        ...
                        <h4 class='font-classic fw-bold mb-0 article-title'>{p.ArticleTitle}</h4>
        ...
        protected void btnDoPreUpload_Click(object sender, EventArgs e)
        {
            ...
            BindPracticesByCid(int.Parse(Request.QueryString["cid"]));
        }
